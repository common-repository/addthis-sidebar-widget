...
class AddThisBookmarkWidget {
  ...
  function display($args) {
    ...
    if ($title)
      echo $before_title . $title . $after_title;
    echo '<div style="text-align:center"><a href="'.$url.'" '.$window.' title="' . __('Bookmark using any bookmark manager!') . '"><img src="'.$img['url'].'"  width="'.$img['width'].'" height="'.$img['height'].'" alt="' . __('AddThis Social Bookmark Button') . '" /></a></div>';
    echo $after_widget;
  }
